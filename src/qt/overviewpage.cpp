...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizeboomAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeboomAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeboomAmount = strAnonymizeboomAmount.remove(strAnonymizeboomAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeboomAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeboomAmount * COIN) nMaxToAnonymize = nAnonymizeboomAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeboomAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeboomAmount));
        strAnonymizeboomAmount = strAnonymizeboomAmount.remove(strAnonymizeboomAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeboomAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
