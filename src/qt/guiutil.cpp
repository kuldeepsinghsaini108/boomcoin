...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a boom address (e.g. %1)").arg("D7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
#endif
...
{
    // return if URI is not valid or is no boom: URI
...
{
    // Convert boom:// to boom:
    //
    //    Cannot handle this later, because boom:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open boom.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "boom.lnk";
}
...
{
    return GetAutostartDir() / "boom.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a boom.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=boom\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the boom app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add boom app to startup item list
