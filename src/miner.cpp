...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("boomMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("boomMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("boomMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("boom-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running boomMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
